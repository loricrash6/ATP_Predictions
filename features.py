...
def create_pre_match_features(row):
	...
	avg_p1=retrieve_player_stats(player1,player2,date,r)
	avg_p2=retrieve_player_stats(player2,player1,date,r)
	...
	if ((avg_p1.shape[0]>0) and (avg_p2.shape[0]>0)):
		...
		#h2h
		h2h_1=df[((df["winner_name"]==player1) & (df["loser_name"]==player2)) & ((df["tourney_date"]<datetime.strptime(date,"%Y-%m-%d")) | (\
			(df["tourney_date"]==datetime.strptime(date,"%Y-%m-%d")) & (df["round"]<r)))].shape[0]
		h2h_2=df[((df["winner_name"]==player2) & (df["loser_name"]==player1)) & ((df["tourney_date"]<datetime.strptime(date,"%Y-%m-%d")) | (\
			(df["tourney_date"]==datetime.strptime(date,"%Y-%m-%d")) & (df["round"]<r)))].shape[0]
...
def retrieve_player_stats(player1,player2,date,r):
	"""
	...
	#print("Retrieving data about {} with respect to {} and before {}...".format(player1,player2,date))
	...
	g1=df[((df["winner_name"]==player1) | (df["loser_name"]==player1)) & ((df["tourney_date"]<datetime.strptime(date,"%Y-%m-%d")) | (\
		(df["tourney_date"]==datetime.strptime(date,"%Y-%m-%d")) & (df["round"]<r)))]
	...
	g2=df[((df["winner_name"]==player2) | (df["loser_name"]==player2)) & ((df["tourney_date"]<datetime.strptime(date,"%Y-%m-%d")) | (\
		(df["tourney_date"]==datetime.strptime(date,"%Y-%m-%d")) & (df["round"]<r)))]
	#o1=def1.loc[def1["winner_name"]==player1]
	...
	for o in co:
		...
		((df["tourney_date"]<datetime.strptime(date,"%Y-%m-%d")) | ((df["tourney_date"]==datetime.strptime(date,"%Y-%m-%d")) & (df["round"]<r)))]
		if m.shape[0] > 0:
			...
			'tourney_date']].rename(columns={'w_fs':'fs','w_w1s':'w1s','w_w2s':'w2s','w_wsp':'wsp','w_wrp':'wrp','w_tpw':'tpw','w_apg':'apg','w_dfpg':'dfpg','w_bppg':'bppg',\
			'w_bps':'bps','l_bppg':'bpo','l_bps':'l_bps','loser_name':'opponent', 'tourney_date':'date'})
			if w.shape[0]>0:
				w["bpc"]=w.apply(lambda row: 1-row["l_bps"],axis=1)
				#set year difference param.
				w["year_diff"]=w.apply(lambda row: int(datetime.strptime(date, '%Y-%m-%d').year-row["date"].year), axis=1)
			...
			'tourney_date']].rename(columns={'l_fs':'fs','l_w1s':'w1s','l_w2s':'w2s','l_wsp':'wsp','l_wrp':'wrp','l_tpw':'tpw','l_apg':'apg','l_dfpg':'dfpg','l_bppg':'bppg',\
			'l_bps':'bps','w_bppg':'bpo','w_bps':'w_bps','winner_name':'opponent','tourney_date':'date'})
			if l.shape[0]>0:
				l["bpc"]=l.apply(lambda row: 1-row["w_bps"],axis=1)
				
				l["year_diff"]=l.apply(lambda row: int(datetime.strptime(date, '%Y-%m-%d').year-row["date"].year), axis=1)
...
if __name__ == '__main__':
	...
	#print(df.shape)
	column_names=["p1","p2","rank","fs","w1sp","w2sp","wsp","wrp","tpw","aces","df","bpc","bps","bpo","bpw","tmw","complete","serveadv","h2h","uncertainty","winner"]
	...
	count=0
	for i in range(10000,10001):#,df.shape[0]):
		#print(i)
		row=df.loc[i,:]
